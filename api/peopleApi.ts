...
import { PersonResponseSchema } from '../model/personResponseSchema';
...
export class PeopleApi {
    ...
    public setApiKey(key: PeopleApiApiKeys, value: string) {
        (this.authentications as any)[PeopleApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getPerson (id: string, select?: string, userAgent?: any, mailto?: any, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PersonResponseSchema;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PersonResponseSchema;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "PersonResponseSchema");
